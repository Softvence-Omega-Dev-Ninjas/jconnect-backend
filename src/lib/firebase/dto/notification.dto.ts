...
export class NotificationContent {
    ...
    @ApiPropertyOptional({ description: 'Notification image URL', example: 'https://example.com/image.jpg' })
    @IsOptional()
    @IsString()
    imageUrl?: string;
}
...
export class AndroidConfig {
    @ApiPropertyOptional({ description: 'Android notification priority', example: 'high', enum: ['high', 'normal'] })
    @IsOptional()
    @IsString()
    ...
    @ApiPropertyOptional({ description: 'Android notification channel ID', example: 'default_channel' })
    @IsOptional()
    @IsString()
    channelId?: string;
...
export class ApnsConfig {
    @ApiPropertyOptional({ description: 'APNS alert object', example: { title: 'Title', body: 'Body' } })
    @IsOptional()
    @IsObject()
    alert?: {
        title?: string;
...
export class WebPushConfig {
    ...
    @ApiPropertyOptional({ description: 'Web notification body', example: 'You have a new message' })
    @IsOptional()
    @IsString()
    body?: string;
...
export class SendNotificationDto {
    ...
    @ApiPropertyOptional({ description: 'Custom data payload', example: { userId: '123', type: 'message' } })
    @IsOptional()
    @IsObject()
...
export class TopicSubscriptionDto {
    @ApiProperty({ description: 'Array of FCM tokens', type: [String], example: ['token1', 'token2'] })
    @IsArray()
    @IsString({ each: true })
    tokens: string[];
...
export class UpdateFcmTokenDto {
    ...
    @ApiPropertyOptional({ description: 'Device platform', example: 'android', enum: ['android', 'ios', 'web'] })
    @IsOptional()
    @IsString()
